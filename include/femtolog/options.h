...
struct FemtologOptions {
   ...
   */
  size_t backend_format_buffer_size = 1024 * 2;
   ...
   */
  size_t backend_dequeue_buffer_size = 1024 * 4;
   ...
   * `std::numeric_limits<size_t>::max()` to disable CPU affinity.
...
constexpr FemtologOptions kMemorySavingOptions{
    1024, 256, 512, std::numeric_limits<size_t>::max(), ColorMode::kAuto,
};

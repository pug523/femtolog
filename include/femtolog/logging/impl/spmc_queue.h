...
class FEMTOLOG_LOGGING_EXPORT SpmcQueue {
 public:
  ...
  SpmcQueueStatus dequeue_bytes(void* data_ptr, size_t data_size) noexcept;
  ...
  [[nodiscard]] inline bool empty() const noexcept {
    const size_t head = head_idx_.load(std::memory_order_acquire);
 ...
 private:
  [[nodiscard]] static constexpr size_t next_power_of_2(
      size_t n) noexcept;
  ...
  alignas(core::kCacheSize) std::atomic<size_t> tail_idx_ = 0;

  // Consumer side (read-mostly, separate cache line)
  // Using ticket-based approach for multiple consumers
  alignas(core::kCacheSize) std::atomic<size_t> head_idx_ = 0;
  alignas(core::kCacheSize) std::atomic<size_t> commit_idx_ = 0;
...
// static
constexpr size_t SpmcQueue::next_power_of_2(size_t n) noexcept {
  if (n <= 1) {
    return 2;
  }
  if ((n & (n - 1)) == 0) {

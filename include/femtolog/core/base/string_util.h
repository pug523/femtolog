...
FEMTOLOG_CORE_EXPORT void to_lower(char* input, size_t len);
...
FEMTOLOG_CORE_EXPORT void to_upper(char* input, size_t len);
...
[[nodiscard]] FEMTOLOG_CORE_EXPORT size_t utf8_char_length(
    unsigned char lead);
...
[[nodiscard]] FEMTOLOG_CORE_EXPORT std::string remove_bracket(
    const std::string& input,
    size_t max_nest_size = 32);
[[nodiscard]] FEMTOLOG_CORE_EXPORT size_t safe_strlen(const char* str);
...
FEMTOLOG_CORE_EXPORT size_t write_raw(char*& dest,
                                           const char* source,
                                           size_t len);
...
size_t write_format(char*& cursor,
  ...
  const auto result =
      std::format_to_n(cursor, remaining - 1, fmt,
...
inline void write_bytes(char*& cursor, const void* data, size_t size) {
  std::memcpy(cursor, data, size);
  cursor += size;
}

inline void read_bytes(const char*& cursor, void* data, size_t size) {

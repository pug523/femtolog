...
#define FEMTOLOG_CHECK_IMPL(condition, type)                                  \
  ...
  ::femtolog::base::CheckFailureStream(#type, __FILE__, __LINE__, #condition) \
      .stream()

#define FEMTOLOG_CHECK_W_OP_IMPL(val1, val2, op, type)                       \
  __builtin_expect(!(static_cast<bool>((val1)op(val2))), 0)                  \
      ? ::femtolog::base::log_check_failure(#type, __FILE__, __LINE__,       \
                                            #val1 " " #op " " #val2, (val1), \
                                            (val2))                          \
      : ::femtolog::base::null_stream()
...
std::ostream& log_check_failure(const char* type,
  ...
  if constexpr (base::is_ostreamable_v<R> && base::is_ostreamable_v<L>) {
    os << "  expected: " << rhs << "\n"
       << "    actual: " << lhs << "\n";
  } else {

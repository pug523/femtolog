...
class MockSink : public SinkBase {
 public:
  ...
  MOCK_METHOD(void,
              on_log,
              (const LogEntry& entry, const char* content, size_t len),
              (override));
  ...
  void capture_log(const LogEntry& entry, const char* content, size_t len) {
    LogData data;
...
TEST_F(InternalLoggerTest, LiteralStringLogging) {
  ...
  EXPECT_CALL(*mock_sink_ptr_, on_log(_, _, _))
      .WillOnce([this](const LogEntry& entry, const char* content, size_t len) {
...
TEST_F(InternalLoggerTest, MultipleSinks) {
  ...
  EXPECT_CALL(*sink1_ptr, on_log(_, _, _))
      .WillOnce(
          [sink1_ptr](const LogEntry& entry, const char* content, size_t len) {
            sink1_ptr->capture_log(entry, content, len);
          });

  EXPECT_CALL(*sink2_ptr, on_log(_, _, _))
      .WillOnce(
          [sink2_ptr](const LogEntry& entry, const char* content, size_t len) {
...
TEST_F(InternalLoggerTest, DifferentLogLevels) {
  ...
  EXPECT_CALL(*mock_sink_ptr_, on_log(_, _, _))
      .Times(6)
      .WillRepeatedly(
          [this](const LogEntry& entry, const char* content, size_t len) {
            mock_sink_ptr_->capture_log(entry, content, len);
          });
...
TEST_F(InternalLoggerTest, RealisticLoggingScenario) {
  ...
  if (mock_sink_ptr_->captured_logs.size() > 1) {
    for (size_t i = 1; i < mock_sink_ptr_->captured_logs.size(); ++i) {
